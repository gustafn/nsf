...
int canonicalNonpositionalArgs(parseContext *pcPtr, Tcl_Interp *interp, int objc,  Tcl_Obj *CONST objv[]);
#endif
...
*/
# define FRAME_IS_XOTCL_METHOD 0x10000
# define FRAME_IS_XOTCL_OBJECT 0x20000
#else 
...
void tcl85showStack(Tcl_Interp *interp) {
  ...
  for (; framePtr; framePtr = Tcl_CallFrame_callerPtr(framePtr)) {
    fprintf(stderr, "... var frame %p flags %.6x cd %.8p ns %.8p %s objv[0] %s\n",
            framePtr, Tcl_CallFrame_isProcCallFrame(framePtr), 
            Tcl_CallFrame_clientData(framePtr),
...
nonXotclObjectProcFrame(Tcl_CallFrame *framePtr) {
  for (; framePtr; framePtr = Tcl_CallFrame_callerPtr(framePtr)) {
    if (Tcl_CallFrame_isProcCallFrame(framePtr) & FRAME_IS_PROC) break;
    if ((Tcl_CallFrame_isProcCallFrame(framePtr) & FRAME_IS_XOTCL_OBJECT) == 0) break;
  }
  return framePtr;
}
Tcl_Namespace *
currentNonFakeNamespace(Tcl_Interp *interp) {
  CallFrame *varFramePtr = (CallFrame *)Tcl_Interp_varFramePtr(interp);
  for (; varFramePtr; varFramePtr = Tcl_CallFrame_callerPtr(varFramePtr)) {
    fprintf(stderr, "nsptr %p fake %p flags  %.6x\n",varFramePtr->nsPtr,RUNTIME_STATE(interp)->fakeNS,Tcl_CallFrame_isProcCallFrame(varFramePtr));

    if (Tcl_CallFrame_isProcCallFrame(varFramePtr) & FRAME_IS_XOTCL_OBJECT) continue;
    break;
    /*if (varFramePtr->nsPtr == RUNTIME_STATE(interp)->fakeNS)
      continue;*/
  }
  return varFramePtr ? varFramePtr->nsPtr : NULL;
}
#else
Tcl_CallFrame * nonXotclObjectProcFrame(Tcl_CallFrame *framePtr) {return framePtr;}
Tcl_Namespace *currentNonFakeNamespace(Tcl_Interp *interp) {return Tcl_GetCurrentNamespace(interp);}
#endif
...
GetSelfObj(Tcl_Interp *interp) {
  ...
  /*fprintf(stderr, "interp has frame %p and varframe %p\n",
...
# endif
  /* Skip frames of type FRAME_IS_XOTCL_OBJECT 
   */
  for (varFramePtr = inFramePtr; 
       varFramePtr && (Tcl_CallFrame_isProcCallFrame(varFramePtr) & FRAME_IS_XOTCL_OBJECT) ; 
  ...
  fprintf(stderr,"active %p, top %p, varFrame(interp) %p, topVarFrame %p, active->curr %p\n",
          active, top, inFramePtr, top->currentFramePtr,
  ...
  if (active == top || inFramePtr == NULL || Tcl_CallFrame_level(inFramePtr) == 0) {
    ...
    if (inFramePtr == varFramePtr) {
      ...
      Tcl_Interp_varFramePtr(interp) = (CallFrame *)varFramePtr;
      ctx->framesSaved = 1;
    }
  } else if (active == NULL) {
    ...
    fprintf(stderr, "... save frame with top proc, varFrame %p callframe %p\n",cf,inFramePtr);
    ctx->varFramePtr = inFramePtr;
    ...
    fprintf(stderr, "... save frame from deeper active frame, varFrame %p callframe %p\n",inFramePtr,framePtr);
    ctx->varFramePtr = inFramePtr;
...
CallStackPush(Tcl_Interp *interp, XOTclObject *obj, XOTclClass *cl,
  ...
  /* if CallStackPush() is called with objc==0, this means that the
     xotcl context of a cmd (e.g. subst) should be the given object
     (e.g. self should resolve). We could do at least 3 things:
     (1) push here a tcl frame, which could cause confusions with uplevel
     (2) make the current object not only accessible via the FRAME_IS_XOTCL_METHOD
         of the stack, but get it differently (a new frame type FRAME_IS_XOTCL_OBJECT)
     (3) patch (and unpatch in pop) the clientdata and flags
  */
#endif
...
GuardCall(XOTclObject *obj, XOTclClass *cl, Tcl_Command cmd,
  ...
  if (guard) {
    ...
    /*csc->callType |= XOTCL_CSC_CALL_IS_GUARD; XXXX TCL85STACK TODO */
...
PushProcCallFrame(ClientData clientData, register Tcl_Interp *interp, int objc,	Tcl_Obj *CONST objv[], 
                  int isLambda, XOTclCallStackContent *csc)	{
...
# endif
  ...
  result = TclPushStackFrame(interp, (Tcl_CallFrame **) framePtrPtr,
			     (Tcl_Namespace *) nsPtr,
			     (isLambda? (FRAME_IS_PROC|FRAME_IS_LAMBDA|FRAME_IS_XOTCL_METHOD) :
...
callProcCheck(ClientData cp, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[],
              ...
              int frameType, int isTclProc, XOTclCallStackContent *csc) {
  int result = TCL_OK;
  ...
  */
  ...
  if (isTclProc == 0) {
    ...
    if (csc) {
       ...
       * TODO: maybe push should happend alread before assertion checking,
    ...
    */
     ...
     */
    /*fprintf(stderr,"calling proc %s isTclProc %d tearDown %p\n",methodName,isTclProc,obj->teardown);*/
...
# if defined(CANONICAL_ARGS)
    ...
    {
      parseContext pc;
      int rc = canonicalNonpositionalArgs(&pc, interp, objc, objv);
      if (rc == TCL_CONTINUE) {
	result = PushProcCallFrame(cp, interp, objc, objv, /*isLambda*/ 0, csc);
        ...
        {int j;
          for(j=0; j<pc.objc+1; j++) {
            ...
            fprintf(stderr, " o[%d]=%p %s,", j, pc.full_objv[j], pc.full_objv[j] ? ObjStr(pc.full_objv[j]) : "NADA");
          }
          fprintf(stderr,"\n");
        }
#endif
...
DoCallProcCheck(ClientData clientData, Tcl_Interp *interp,
  ...
  /*fprintf(stderr, "DoCallProcCheck method '%s' cmd %p cp=%p objc=%d\n",methodName,cmd, cp, objc);*/
  ...
  /*fprintf(stderr,"... DoCallProcCheck %s push=%d, obj=%s\n", methodName, push, objectName(obj));*/
  ...
  {  
    ...
    rc = callProcCheck(cp, interp, objc, objv, cmd, obj, cl,
                       methodName, frameType, isTclProc, push ? cs->top : NULL);
  }
  if (push) {
    CallStackPop(interp);
  }
...
NonposArgsDeleteHashEntry(Tcl_HashEntry *hPtr) {
  ...
  if (nonposArg) {
    ...
    if (nonposArg->ifd) {
      argDefinitionsFree(nonposArg->ifd);
    }
...
parseArgDefinition(Tcl_Interp *interp, char *procName, Tcl_Obj *arg, int isNonposArgument,
  ...
  if (isNonposArgument) {
    ...
    argName = argString;
    nameLength = length;
    ifPtr->required = 1; /* positional arguments are required unless we have a default */
  }
   ...
   */
  if (ifPtr->required == 0 && ifPtr->defaultValue == NULL) {
    (*possibleUnknowns)++;
  }
  return TCL_OK;
}
...
parseNonposArgs(Tcl_Interp *interp, char *procName, Tcl_Obj *npArgs, Tcl_Obj *ordinaryArgs,
                ...
                XOTclParsedInterfaceDefinition *parsedIfPtr) {
...
MakeProc(Tcl_Namespace *ns, XOTclAssertionStore *aStore, Tcl_HashTable **nonposArgsTable,
  ...
  if (haveNonposArgs) {
    ...
    result = parseNonposArgs(interp, procName, nonposArgs, ordinaryArgs,
                             nonposArgsTable, &haveNonposArgs, &parsedIf);
...
# if defined(CANONICAL_ARGS)
    ...
    for (aPtr = parsedIf.ifd; aPtr->name; aPtr++) {
      if (*aPtr->name == '-') {
...
callingNameSpace(Tcl_Interp *interp) {
  ...
  fprintf(stderr," **** callingNameSpace: use last invocation csc = %p\n",  csc);
  if (csc && csc->currentFramePtr) {
    /* use the callspace from the last invocation */
    XOTclCallStackContent *called = csc<top? csc+1 : NULL;
    Tcl_CallFrame *framePtr = called ? Tcl_CallFrame_callerPtr(called->currentFramePtr) : NULL;
    fprintf(stderr,"      csc use frame= %p\n",  framePtr);
    if (framePtr) {
      nsPtr = framePtr->nsPtr;
    } else {
      framePtr = nonXotclObjectProcFrame((Tcl_CallFrame *)Tcl_Interp_varFramePtr(interp));
      ...
      while (nsPtr == RUNTIME_STATE(interp)->XOTclNS) {
        ...
        /*fprintf(stderr, "... new ns %s\n",nsPtr->fullName);*/
      }
      fprintf(stderr, "      found ns %p '%s'\n", nsPtr, nsPtr?nsPtr->fullName:nsPtr);
    }
  }
  if (!nsPtr) {
    /* calls on xotcl toplevel */
    XOTclCallStackContent *bot = cs->content + 1;
    fprintf(stderr, "      TOPLEVEL bot=%p diff=%d\n", bot, top-bot);
    if (top - bot >= 0 && bot->currentFramePtr) {
      /* get calling tcl environment */
      Tcl_CallFrame *framePtr = Tcl_CallFrame_callerPtr(bot->currentFramePtr);
      if (framePtr) {
        nsPtr = framePtr->nsPtr;
        /*fprintf(stderr, "top=%p, bot=%p b->c=%p f=%p ns=%p\n",
          top, bot, bot->currentFramePtr, framePtr, ns);*/
...
parseObjv(Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[], Tcl_Obj *procName,
  ...
  for (i=0, o=1, aPtr=ifdPtr; aPtr->name && o<objc;) {
    ...
    fprintf(stderr,"... (%d) processing [%d]: '%s' %s\n",i, o,aPtr->name,aPtr->required?"req":"not req");
#endif
    if (*aPtr->name == '-') {
        ...
        if (objStr[0] == '-') {
          ...
          for (bPtr = aPtr; bPtr->name && *bPtr->name == '-'; bPtr ++) {
            if (strcmp(objStr,bPtr->name) == 0) {
              ...
              if (bPtr->required) nrReq++; else nrOpt++;
              if (bPtr->nrargs == 0) {
...
#if defined(PARSE_TRACE)
  fprintf(stderr, "END lastobjc %d, varargs %d, not enough args (%d<%d) = %d, to many (%d>%d) = %d\n",
...
canonicalNonpositionalArgs(parseContext *pcPtr, Tcl_Interp *interp, int objc,  Tcl_Obj *CONST objv[]) {
  ...
  /* Set objc always to the size of the interface */
