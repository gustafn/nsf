...
DoCallProcCheck(ClientData clientData, Tcl_Interp *interp,
  ...
  if (cp) {
    ...
    /* push the xotcl info */
    if (!(csc = CallStackPush(interp, obj, cl, cmd, objc, objv, frameType))) {
      return TCL_ERROR;
    }
  ...
  /*fprintf(stderr,"... DoCallProcCheck %s csc=%p, obj=%s\n", methodName, csc, objectName(obj));*/
...
DoDispatch(ClientData clientData, Tcl_Interp *interp, int objc,
  ...
  if (cmd) {
	...
	(flags & XOTCL_CM_NO_UNKNOWN) == 0) {
      XOTclObject *o = NULL, *self = GetSelfObj(interp);
...
int
XOTclInstvarCmd(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
