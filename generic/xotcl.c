...
DoDispatch(ClientData clientData, Tcl_Interp *interp, int objc, 
  ...
  /*fprintf(stderr,"DoDispatch obj = %s objc = %d 0=%s methodName=%s\n",
    ObjStr(obj->cmdName), objc, ObjStr(objv[0]), methodName);*/
...
static XOTclClass *
DefaultSuperClass(Tcl_Interp *interp, XOTclClass *cl, XOTclClass *mcl, XOTclClass *topcl) {
  XOTclClass *defaultClass = topcl;

  /*fprintf(stderr, "DefaultSuperClass cl %s, mcl %s\n",
          ObjStr(cl->object.cmdName),
          mcl ? ObjStr(mcl->object.cmdName) : "NULL"
          );*/
  if (mcl) {
    int result;
    result = setInstVar(interp, (XOTclObject *)mcl, 
	...
	DECR_REF_COUNT(nameObj);
        ...
        for (sc = mcl->super; sc && sc->cl != cl; sc = sc->nextPtr) {
          ...
          result = DefaultSuperClass(interp, cl, sc->cl, topcl);
          if (result != topcl) {
            return result;
          }
        }
...
CleanupDestroyClass(Tcl_Interp *interp, XOTclClass *cl, int softrecreate, int recreate) {
  ...
  */
  ...
  if (!softrecreate) {
    ...
    defaultClass = DefaultSuperClass(interp, cl, cl->object.cl, RUNTIME_STATE(interp)->theObject);
    ...
    */
    if (cl != theobj) {
      XOTclClass *baseClass = IsMetaClass(interp, cl) ? RUNTIME_STATE(interp)->theClass : defaultClass;
      if (baseClass == cl) {
...
CleanupInitClass(Tcl_Interp *interp, XOTclClass *cl, Tcl_Namespace *namespacePtr,
  ...
  */
  ...
  defaultSuperclass = DefaultSuperClass(interp, cl, cl->object.cl, RUNTIME_STATE(interp)->theObject);
    /*
...
setInstVar(Tcl_Interp *interp, XOTclObject *obj, Tcl_Obj *name, Tcl_Obj *value) {
  ...
  if (value == NULL) {
    ...
    /*fprintf(stderr,"setvar in obj %s: name %s = %s\n",ObjStr(obj->cmdName),ObjStr(name),ObjStr(value));*/
...
XOTclClassInfoSuperclassMethod(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (args < 2 || args > 3) return XOTclObjErrArgCnt(interp, objv[0],  NULL, "<class> ?pattern?");  
  if (GetXOTclClassFromObj(interp, objv[1], &cl, 0) != TCL_OK)
    return XOTclObjErrType(interp, objv[1], "Class");
