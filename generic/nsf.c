...
NsfCallUnkownHandler(Tcl_Interp *interp, Tcl_Obj *nameObj) {
  ...
  /*fprintf(stderr, "try ::nsf::unknown for '%s'\n", ObjStr(nameObj));*/
 ...
 *    the unknown function (::nsf::unknown) to fetch the class on demand and
 *    retries the conversion.  On success the NsfClass is returned in the
 *    third argument. The objPtr might be converted by this function.
 *
 * Results:
 *    True or false,
 ...
 *    tries to load a parent object via ::nsf::unknown.
 *
 * Results:
 *    returns 1 on success
 *
...
NsfOConfigureMethod(Tcl_Interp *interp, NsfObject *object, int objc, Tcl_Obj *CONST objv[]) {
  ...
  for (i=1, paramPtr = paramDefs->paramsPtr; paramPtr->name; paramPtr++, i++) {
    ...
    if ((object->flags & NSF_INIT_CALLED) && (pc.flags[i-1] & NSF_PC_IS_DEFAULT)) {
      ...
      /*
       * Object parameter method calls (NSF_ARG_METHOD_INVOCATION set) do not
       * set instance variables, so we do not have to check for existing
       * variables.
       */
