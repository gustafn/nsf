/* $Id: xotcl.c,v 1.50 2007/10/05 09:06:00 neumann Exp $
 *
...
XOTCLINLINE static Tcl_Command NSFindCommand(Tcl_Interp *in, char *name, Tcl_Namespace *ns);
...
XOTcl_Object*
XOTclGetSelfObj(Tcl_Interp *in) {
  return (XOTcl_Object*)GetSelfObj(in);
}
...
SetXOTclObjectFromAny(Tcl_Interp *in, register Tcl_Obj *objPtr) {
  ...
  if (!isAbsolutePath(string)) {
    ...
    obj = XOTclpGetObject(in, nsString);
    ...
    if (!obj) {
      ...
      INCR_REF_COUNT(tmpName);
      obj = XOTclpGetObject(in, ObjStr(tmpName));
      DECR_REF_COUNT(tmpName);
    }
  } else {
    obj = XOTclpGetObject(in, string);
  }
...
NSDeleteChildren(Tcl_Interp *in, Tcl_Namespace* ns) {
  ...
  Tcl_ForgetImport(in, ns, "*"); /* don't destroy namespace imported objects */
...
XOTclGetObject(Tcl_Interp *in, char *name) {
  return (XOTcl_Object*) XOTclpGetObject(in, name);
}
 ...
 */
static XOTclObject*
XOTclpGetObject(Tcl_Interp *in, char *name) {
...
XOTclGetClass(Tcl_Interp *in, char *name) {
  return (XOTcl_Class*)XOTclpGetClass(in, name);
}

static XOTclClass*
XOTclpGetClass(Tcl_Interp *in, char *name) {
  XOTclObject *obj = XOTclpGetObject(in, name);
...
AutonameIncr(Tcl_Interp *in, Tcl_Obj *name, XOTclObject *obj,
  ...
  valueObject = Tcl_ObjGetVar2(in, XOTclGlobalObjects[XOTE_AUTONAMES],name, flgs);
  if (valueObject != NULL ) {
  ...
  Tcl_ObjSetVar2(in, XOTclGlobalObjects[XOTE_AUTONAMES], name, 
                 valueObject, flgs);
  
  if (resetOpt) {
    if (valueObject != NULL) { /* we have an entry */
      Tcl_UnsetVar2(in, XOTclGlobalStrings[XOTE_AUTONAMES], ObjStr(name), flgs);
    }
...
CallStackDoDestroy(Tcl_Interp *in, XOTclObject *obj) {
  ...
  obj->id = 0;
  if (obj->teardown && oid) {
      /* This command will call PrimitiveODestroy or PrimitiveCDestroy */
 ...
 */
static void
MixinInvalidateObjOrders(Tcl_Interp *in, XOTclClass *cl) {
...
getFullProcQualifier(Tcl_Interp *in, CONST84 char *cmdName,
  ...
  if (cl) {
    ...
    if (isTcl) {
      Tcl_ListObjAppendElement(in, list, XOTclGlobalObjects[XOTE_INSTPROC]);
    } else if (objProc == XOTclForwardMethod) {
      Tcl_ListObjAppendElement(in, list, XOTclGlobalObjects[XOTE_INSTFORWARD]);
    ...
    if (isTcl) {
      Tcl_ListObjAppendElement(in, list, XOTclGlobalObjects[XOTE_PROC]);
    } else if (objProc == XOTclForwardMethod) {
      Tcl_ListObjAppendElement(in, list, XOTclGlobalObjects[XOTE_FORWARD]);
    } else if (objProc == XOTclSetterMethod) {
      Tcl_ListObjAppendElement(in, list, XOTclGlobalObjects[XOTE_PARAMETERCMD]);
    } else {
      Tcl_ListObjAppendElement(in, list, XOTclGlobalObjects[XOTE_CMD]);
    }
  }
...
static int
SuperclassAdd(Tcl_Interp *in, XOTclClass *cl, int oc, Tcl_Obj **ov, Tcl_Obj *arg) {
  ...
  if (!ComputeOrder(cl, cl->order, Super)) {
    ...
    return XOTclErrBadVal(in, "superclass", "a cycle-free graph", ObjStr(arg));
  }
  XOTclFreeClasses(osl);
...
DoCallProcCheck(ClientData cp, ClientData cd, Tcl_Interp *in,
  ...
  rc = callProcCheck(cp, in, objc, objv, cmd, obj, cl,
...
XOTCLINLINE static int
DoDispatch(ClientData cd, Tcl_Interp *in, int objc, 
           Tcl_Obj *CONST objv[], int flags) {
...
MakeProc(Tcl_Namespace *ns, XOTclAssertionStore *aStore,
  ...
  Tcl_PushCallFrame(in,(Tcl_CallFrame *)framePtr,ns,0);

  result = Tcl_ProcObjCmd(0, in, 4, ov) != TCL_OK;
#if defined(NAMESPACEINSTPROCS)
  {
    Proc *procPtr = TclFindProc((Interp *)in, procName);
...
ListInfo(Tcl_Interp *in, int isclass) {
  ...
  Tcl_AppendElement(in, "precedence");
  if (isclass) {
...
# endif
  ...
  for (i=0; i<length; i++) {
    ...
    if (varExists(in, obj, ObjStr(element), NULL, 0, 1)) {
...
ListObjPtrHashTable(Tcl_Interp *in, Tcl_HashTable *table, char *pattern) {
  Tcl_HashEntry* hPtr;
  if (pattern && noMetaChars(pattern)) {
    XOTclObject *childobj = XOTclpGetObject(in, pattern);
...
ListSubclasses(Tcl_Interp *in, XOTclClass *cl, char *pattern) {
  if (pattern == 0) {
    ...
    if (isc == 0) 
      return XOTclErrBadVal(in, "info subclass", "a class", pattern);
    saved = cl->order;
    cl->order = 0;
...
static int
ListPrecedence(Tcl_Interp *in, XOTclObject *obj, char *pattern) {
  XOTclClasses* pl;
  Tcl_ResetResult(in);
  if (!(obj->flags & XOTCL_MIXIN_ORDER_VALID))
    MixinComputeDefined(in, obj);
...
static int
ListArgsFromOrdinaryArgs(Tcl_Interp *in, XOTclNonposArgs* nonposArgs) {
  ...
  for (i=0; i < ordinaryArgsDefc; i++) {
    ...
    Tcl_ListObjAppendElement(in, argList, ordinaryArg);
  }
  Tcl_SetObjResult(in, argList);
  return TCL_OK;
}
...
static int
SetProcDefault(Tcl_Interp *in, Tcl_Obj *var, Tcl_Obj* defVal) {
  int result = TCL_OK;
  ...
  if (result == TCL_ERROR) {
    XOTclVarErrMsg(in, "couldn't store default value in variable '",
                   var, "'", (char *) NULL);
  }
  return result;
}
...
ListProcDefault(Tcl_Interp *in, Tcl_HashTable *table,
  ...
  if (GetProcDefault(in, table, name, arg, &defVal) == TCL_OK) {
    result = SetProcDefault(in, var, defVal);
  } else {
    XOTclVarErrMsg(in, "method '", name,
...
ListProcBody(Tcl_Interp *in, Tcl_HashTable *table, char *name) {
  ...
  if (proc) {
    ...
    Tcl_SetObjResult(in, Tcl_NewStringObj(StripBodyPrefix(body), -1));
    return TCL_OK;
  }
  return XOTclErrBadVal(in, "info body", "a tcl method name", name);
}
...
int
XOTclQualifyObjCmd(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  char *string;
  if (objc != 2)
    return XOTclVarErrMsg(in, "wrong # of args for __qualify", (char *) NULL);
  
  string = ObjStr(objv[1]);
  if (!isAbsolutePath(string)) {
    Tcl_SetObjResult(in, NameInNamespaceObj(in,string,callingNameSpace(in)));
  } else {
...
XOTclONextMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (csc<cs->content)
    return XOTclVarErrMsg(in, "__next: can't find object",
...
XOTclSelfSubCommand(Tcl_Interp *in, XOTclObject *obj, char *option) {
  ...
  if (isProcString(option)) { /* proc subcommand */
    ...
    Tcl_SetObjResult(in, cl ? cl->object.cmdName : XOTclGlobalObjects[XOTE_EMPTY]);
    return TCL_OK;
  } else {
  ...
  return XOTclVarErrMsg(in, "unknown option '", option, 
                        "' for self", (char *) NULL);
}
 ...
 */
static void
CleanupDestroyObject(Tcl_Interp *in, XOTclObject *obj, int softrecreate) {
 ...
 */
static void
PrimitiveCInit(XOTclClass *cl, Tcl_Interp *in, char *name) {
  TclCallFrame frame, *framePtr = &frame;
  Tcl_Namespace* ns;
...
doCleanup(Tcl_Interp *in, XOTclObject *newobj, XOTclObject *classobj,
  ...
  result = changeClass(in, newobj, (XOTclClass*) classobj);

  if (result == TCL_OK) {
    /*
     * dispatch "cleanup"
     */
    result = callMethod((ClientData) newobj, in, XOTclGlobalObjects[XOTE_CLEANUP], 2, 0, 0);
  }
  return result;
}
 ...
 */
static int
doObjInitialization(Tcl_Interp *in, XOTclObject *obj, int objc, Tcl_Obj *CONST objv[]) {
...
XOTclOCleanupMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  softrecreate = obj->flags & XOTCL_RECREATE&& RUNTIME_STATE(in)->doSoftrecreate;

  CleanupDestroyObject(in, obj, softrecreate);
  CleanupInitObject(in, obj, obj->cl, obj->nsPtr, softrecreate);

  if (cl) {
    CleanupDestroyClass(in, cl, softrecreate);
    CleanupInitClass(in, cl, cl->nsPtr, softrecreate);
  }
...
IsMetaClass(Tcl_Interp *in, XOTclClass *cl) {
  ...
  for (mc=mixinClasses; mc; mc = mc->next) {
    ...
    if (isSubType(mc->cl, RUNTIME_STATE(in)->theClass)) {
      hasMCM = 1;
      break;
    }
  }
...
XOTclOIsTypeMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (obj->cl && GetXOTclClassFromObj(in,objv[1],&cl, 1) == TCL_OK) {
    success = isSubType(obj->cl,cl);
  }
  Tcl_ResetResult(in);
  Tcl_SetIntObj(Tcl_GetObjResult(in), success);
  return TCL_OK;
}

static int
hasMixin(Tcl_Interp *in, XOTclObject *obj, XOTclClass *cl) {
...
static int
XOTclOExistsMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
...
XOTclOInfoMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  case 'p':
	...
	if (procs) Tcl_SetObjResult(in, AssertionList(in, procs->post));
      ...
      return ListPrecedence(in, obj, pattern);
    }
    break;
  case 'v':
    if (!strcmp(cmd, "vars")) {
...
XOTclDeleteObject(Tcl_Interp *in, XOTcl_Object *obji) {
  XOTclObject *obj = (XOTclObject*) obji;
  return callMethod((ClientData)obj, in, XOTclGlobalObjects[XOTE_DESTROY],2,0,0);
}

int
XOTclDeleteClass(Tcl_Interp *in, XOTcl_Class *cli) {
  XOTclClass *cl = (XOTclClass*) cli;
  return callMethod((ClientData)cl, in, XOTclGlobalObjects[XOTE_DESTROY],2,0,0);
}
...
static int
XOTclOInstVarMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]);
...
static int 
setInstVar(Tcl_Interp *in, XOTclObject *obj, Tcl_Obj *name, Tcl_Obj* value) {
  Tcl_Obj *result;
  ...
  if (value == NULL) {
    result = Tcl_ObjGetVar2(in, name, NULL, flags);
  } else {
    result = Tcl_ObjSetVar2(in, name, NULL, value, flags);
  }
  XOTcl_PopFrame(in, obj);
...
static int
XOTclOSetMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;

  if (!obj) return XOTclObjErrType(in, objv[0], "Object");
  if (objc > 3)  XOTclObjErrArgCnt(in, obj->cmdName, "set var ?value?");
  return setInstVar(in, obj, objv[1], objc == 3 ? objv[2] : NULL);
}

static int
XOTclSetterMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
...
static int 
callForwarder(forwardCmdClientData *tcd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ClientData cd;
  int result;
  XOTcl_FrameDecls;
  ...
  if (tcd->objProc) {
    ...
    result = Tcl_EvalObjv(in, objc, objv, 0);
  }
...
XOTclOVwaitMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (!foundEvent) {
    return XOTclVarErrMsg(in, "can't wait for variable '", nameString,
...
static int
XOTclOInvariantsMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
  XOTclObjectOpt *opt;
...
static int
XOTclOCheckMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
...
static int
XOTclConfigureCommand(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (objc == 3) {
    result = Tcl_GetBooleanFromObj(in, objv[2], &bool);
  }
  if (result == TCL_OK) {
    switch (opt) {
    case filterIdx:
...
static int
XOTclObjscopedMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  XOTcl_PushFrame(in, obj);
  rc = (tcd->objProc)(tcd->cd, in, objc, objv);
  XOTcl_PopFrame(in, obj);
  return rc;
}
...
XOTclAliasCommand(ClientData cd, Tcl_Interp *in, 
  ...
  if (!cl) {
    ...
    if (!obj) 
      return XOTclObjErrType(in, objv[1], "Class|Object");
    allocation = 'o';
  } else {
    allocation = 'c';
  }
  ...
  if (allocation == 'c') {
    XOTclAddIMethod(in, (XOTcl_Class*)cl, methodName, objProc, tcd, dp);
  } else {
    XOTclAddPMethod(in, (XOTcl_Object*)obj, methodName, objProc, tcd, dp);
  }
  return TCL_OK;
}
...
static int
XOTclSetInstvarCommand(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj;
...
XOTclSetRelationCommand(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (Tcl_GetIndexFromObj(in, objv[2], opts, "relation type", 0, &opt) != TCL_OK) {
    return TCL_ERROR;
  }
  ...
  case mixinIdx: 
    {
      if (objopt->mixins) CmdListRemoveList(&objopt->mixins, GuardDel);
  ...
  case instmixinIdx:
    {
      if (clopt->instmixins) CmdListRemoveList(&clopt->instmixins, GuardDel);
...
static int
XOTclOMixinGuardMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
  XOTclCmdList* h;
  XOTclObjectOpt *opt;
...
static int
XOTclOFilterGuardMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
  XOTclCmdList* h;
  XOTclObjectOpt *opt;
...
static int
XOTclORequireNamespaceMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
...
isDashArg(Tcl_Interp *in, Tcl_Obj *obj, char **methodName, int *objc, Tcl_Obj **objv[]) {
  ...
  */
  ...
  if (obj->typePtr == listType) {
    if (Tcl_ListObjGetElements(in, obj, objc, objv) == TCL_OK && *objc>1) {
      flag = ObjStr(*objv[0]);
...
static int
XOTclOConfigureMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclObject *obj = (XOTclObject*)cd;
...
XOTclCInstDestroyMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (delobj->opt && delobj->opt->volatileVarName) {
      /*
      ...
      */
      /* fprintf(stderr,"volatile var name %s\n",delobj->opt->volatileVarName);*/
      return Tcl_UnsetVar2(in, delobj->opt->volatileVarName, 0, 0);
  }
...
static int
XOTclCAllocMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  XOTclClass *newcl;
  XOTclObject *newobj;
  int result;
...
createMethod(Tcl_Interp *in, XOTclClass *cl, XOTclObject *obj,
   ...
   */
  newobj = XOTclpGetObject(in, objName);
  ...
  if (newobj && (!IsMetaClass(in, cl) || IsMetaClass(in, newobj->cl))) {
...
static int
XOTclCCreateMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  ...
  return createMethod(in, cl, &cl->object, objc, objv);
}
...
static int
XOTclCNewMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass  *cl = XOTclObjectToClass(cd);
  ...
  for (i=1; i<objc; i++) {
    ...
    if (*option == '-' && strcmp(option,"-childof")==0 && i<objc-1) {
      offset += 2;
      if (XOTclObjConvertObject(in, objv[i+1], &child) != TCL_OK) {
...
static int
XOTclCRecreateMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  XOTclObject *newobj;
  int result;
...
forwardProcessOptions(Tcl_Interp *in, int objc, Tcl_Obj * CONST objv[],
  ...
  if (earlybinding) {
    Tcl_Command cmd = Tcl_GetCommandFromObj(in, tcd->cmdName);
    if (cmd == NULL) 
...
static int
XOTclOVolatileMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj * CONST objv[]) {
  XOTclObject *obj = (XOTclObject*) cd;
...
static int
XOTclCInstProcMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  char *argStr, *bdyStr, *name;
  XOTclClassOpt* opt;
  int incr = 0, result = TCL_OK;
...
static int
XOTclCInstFilterGuardMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  XOTclCmdList* h;
  XOTclClassOpt* opt;
...
static int
XOTclCInstMixinGuardMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  XOTclCmdList* h;
...
static int
XOTclCInvariantsMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  XOTclClass *cl = XOTclObjectToClass(cd);
  XOTclClassOpt* opt;
...
XOTcl_NSCopyVars(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  ...
  if (ns) {
    ...
    Tcl_PushCallFrame(in,(Tcl_CallFrame *)framePtr,newNs,0);
  } else {
    XOTclObject *newObj;
...
static int
XOTcl_InterpObjCmd(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *CONST objv[]) {
  Tcl_Interp *slave;
  char *subCmd;
  ALLOC_ON_STACK(Tcl_Obj*,objc, ov);
  ...
  memcpy(ov, objv, sizeof(Tcl_Obj *)*objc);
  if (objc < 1) {
    XOTclObjErrArgCnt(in, NULL, "::xotcl::interp name ?args?");
    goto interp_error;
  }

  ov[0] = XOTclGlobalObjects[XOTE_INTERP];
  if (Tcl_EvalObjv(in, objc, ov, TCL_GLOBAL_ONLY|TCL_LEAVE_ERR_MSG) != TCL_OK) {
    goto interp_error;
  }

  subCmd = ObjStr(ov[1]);
  if (isCreateString(subCmd)) {
    slave = Tcl_GetSlave(in, ObjStr(ov[2]));
    if (!slave) {
      XOTclVarErrMsg(in, "Creation of slave interpreter failed", (char *) NULL);
      goto interp_error;
    }
...
static void deleteProcsAndVars(Tcl_Interp *in) {
  ...
  while (entryPtr != NULL) {
    ...
    if (!TclIsVarUndefined(varPtr) || TclIsVarNamespaceVar(varPtr)) {
      ...
      Tcl_UnsetVar2(in, ObjStr(nameObj), (char *) NULL, TCL_GLOBAL_ONLY);
    }
...
# endif
  ...
  */
  ...
  Tcl_CreateObjCommand(in, "::xotcl::__qualify", XOTclQualifyObjCmd, 0,0);
  Tcl_CreateObjCommand(in, "::xotcl::setinstvar", XOTclSetInstvarCommand, 0, 0);
...
# ifdef COMPILE_XOTCL_STUBS
  Tcl_PkgProvideEx(in, "XOTcl", PACKAGE_VERSION, (ClientData) &xotclStubs);
# else
  Tcl_PkgProvide(in, "XOTcl", PACKAGE_VERSION);
# endif
#endif
