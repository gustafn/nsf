/* $Id: xotcl.c,v 1.10 2004/07/02 11:22:31 neumann Exp $
 *
 *  XOTcl - Extended OTcl
 *
 ...
 *     Altendorferstraﬂe 97-101 *     D-45143 Essen, Germany
 *
...
GetXOTclClassFromObj(Tcl_Interp *in, register Tcl_Obj *objPtr,
  ...
  if (result == TCL_OK) {
    ...
    if (*objName != ':') {
      ov[2] = NameInNamespaceObj(in,objName,Tcl_GetCurrentNamespace(in));
    } else {
      ov[2] = objPtr;
    }
    INCR_REF_COUNT(ov[2]);
...
XOTclCAllocMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj *objv[]) {
  ...
  if (GetXOTclObjectFromObj(in, objv[1], &newobj) == TCL_OK) {
	...
	tmpName = NameInNamespaceObj(in,objName,ns);
...
XOTclCTclCmdMethod(ClientData cd, Tcl_Interp *in, int objc, Tcl_Obj * CONST objv[]) {
  ...
  name = ObjStr(objv[1]);
  if (*name != ':') {
    cmdObj = NameInNamespaceObj(in, name, NULL);
  } else {
    cmdObj = objv[1];
  }
  tcd = NEW(tclCmdClientData);
...
forwardProcessOptions(Tcl_Interp *in, int objc, Tcl_Obj * CONST objv[], 
  ...
  if (tcd->objscope) {
    ...
    */
    ...
    if (*name != ':') {
      tcd->cmdName = NameInNamespaceObj(in, name, NULL);
    }
  }
  INCR_REF_COUNT(tcd->cmdName);
